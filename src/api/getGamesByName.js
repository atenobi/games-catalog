export const getGamesByName = async (name) => {
  ...
  await fetch(`https://eozcxj7i2j3mjjh.m.pipedream.net/games?fields=id,name,release_dates.human,rating,age_ratings.rating,game_engines.name,summary,cover.url,genres.name,platforms.name,game_modes.name,url;search"${name}";#search=name=${name}`, {
    method: "POST",
    headers: {
